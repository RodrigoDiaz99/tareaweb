<x-app-layout title="Prolifearmy | Editar video">
    <div class="p-4">
                    ...
                    @if (Session::has('success'))
                        <div class="bg-green-100 rounded-md p-3 mb-2 flex">
                            ...
                            <div class="text-green-700">
                                <div class="font-bold text-xl">Video Editado</div>
                            </div>
                        </div>
                    @endif
                    <div class="max-w-md mx-auto">
                        <div class="flex items-center space-x-5">
                            ...
                            <div class="block pl-2 font-semibold text-xl self-start text-gray-700">
                                <h2 class="leading-relaxed">Editar Video</h2>
                                <p class="text-sm text-gray-500 font-normal leading-relaxed">Llenar este formulario para
                                    editar un video.</p>
                            </div>
                        </div>
                        <div class="divide-y divide-gray-200">
                            <form action="{{ route('video_update', $video->id) }}" enctype="multipart/form-data"
                                method="POST">
                                @method('PUT')
                                @csrf
                                <!-- path del videoCover a eliminar -->
                                <input type="hidden" name="videoCoverDelete" value="{{ $video->img_path_url }}">
                                ...
                                <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                                    <div class="flex flex-col">
                                        ...
                                        <input type="text" name="name" id="name" onkeyup="mayus(this);" style="text-transform: uppercase;"
                                            ...
                                            value="{{ $video->name }}" placeholder="Nombre del video" required
                                            autofocus>
                                    </div>
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <input type="url" name="link" id="link"
                                            class="px-4 py-2 border focus:ring-gray-500 focus:border-green-500 w-full sm:text-sm border-blue-500 rounded-md focus:outline-none text-gray-600"
                                            value="{{$video->link }}" placeholder="Link del video" >
                                    </div>

                                    <div class="flex flex-col">
                                        <label class="leading-loose">Miniatura</label>
                                        <img class="object-contain h-24 w-full"
                                            src="{{ Storage::url($video->img_path_url) }}" alt="$video->name">
                                        <!-- component -->
                                        <label
